  ...
  return (
    <div key={id}>
      ...
      <div>
        ...
        {reserved ? <button type="button" onClick={() => bookingHandler(id)} className="cancel-button">Cancel Reservation</button> : <button type="button" onClick={() => reservationHandler(id)} className="reserve-button">Reserve Rocket</button>}
      </div>
    </div>
  );
};
